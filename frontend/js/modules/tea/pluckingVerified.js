// ============================================
// TALAEN FARM - Verified Plucking Records (Smart + Approval)
// ============================================
...
class TeaPluckingVerified {
    ...
    static async show() {
        ...
        mainContent.innerHTML = `
            <div class="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div><h1 class="text-2xl font-bold text-slate-800 tracking-tight">Verified Plucking</h1><p class="text-stone-500 text-sm mt-1">Owner verified plucking records</p></div>
                <div class="flex gap-2">
                    <button onclick="TeaPluckingVerified.exportCSV()" class="bg-white border border-stone-200 text-slate-600 px-4 py-2.5 rounded-xl hover:bg-stone-50 transition-all flex items-center gap-2 text-sm font-medium shadow-sm"><i class="fas fa-download"></i> Export</button>
                    <button onclick="TeaPluckingVerified.showAddForm()" class="bg-gradient-to-r from-emerald-600 to-emerald-700 text-white px-5 py-2.5 rounded-xl hover:from-emerald-700 hover:to-emerald-800 transition-all flex items-center gap-2 text-sm font-medium shadow-lg shadow-emerald-600/20"><i class="fas fa-plus"></i> Record Verified Plucking</button>
                </div>
            </div>
            ...
            <div class="bg-white rounded-2xl border border-stone-200 p-4 mb-4 shadow-sm">
                <div class="flex flex-wrap items-center gap-3">
                    <div class="relative flex-1 min-w-[200px]"><i class="fas fa-search absolute left-3.5 top-1/2 -translate-y-1/2 text-stone-400 text-sm"></i><input type="text" id="verifiedSearch" placeholder="Search worker..." class="w-full pl-10 pr-4 py-2.5 bg-stone-50 border border-stone-200 rounded-xl text-sm focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none" oninput="TeaPluckingVerified.filterRecords()"></div>
                    <input type="date" id="vDateFrom" onchange="TeaPluckingVerified.filterRecords()" class="px-3 py-2.5 bg-stone-50 border border-stone-200 rounded-xl text-sm"><span class="text-stone-400 text-sm">to</span><input type="date" id="vDateTo" onchange="TeaPluckingVerified.filterRecords()" class="px-3 py-2.5 bg-stone-50 border border-stone-200 rounded-xl text-sm">
                    <select id="vCompanyFilter" onchange="TeaPluckingVerified.filterRecords()" class="px-3 py-2.5 bg-stone-50 border border-stone-200 rounded-xl text-sm"><option value="">All Companies</option></select>
                    <select id="vApprovalFilter" onchange="TeaPluckingVerified.filterRecords()" class="px-3 py-2.5 bg-stone-50 border border-stone-200 rounded-xl text-sm"><option value="">All Approval</option><option value="approved">✅ Approved</option><option value="disputed">⚠️ Disputed</option><option value="pending">🕐 Pending</option></select>
    ...
    static async loadCompanyFilter() {
        try {
            ...
            if (r.success) { const s = document.getElementById('vCompanyFilter'); r.companies.forEach(c => { const o = document.createElement('option'); o.value = c.id; o.textContent = c.name; s.appendChild(o); }); }
        } catch (e) { console.error('Failed to load companies:', e); }
    }

    static getApprovalBadge(record) {
        const status = record.approval_status || 'pending';
        const config = {
            'approved': { bg: 'bg-emerald-50', text: 'text-emerald-700', border: 'border-emerald-200', icon: 'fa-check-circle', label: 'Approved' },
            'disputed': { bg: 'bg-amber-50', text: 'text-amber-700', border: 'border-amber-200', icon: 'fa-exclamation-triangle', label: 'Disputed' },
            'resolved': { bg: 'bg-sky-50', text: 'text-sky-700', border: 'border-sky-200', icon: 'fa-check-double', label: 'Resolved' },
            'pending': { bg: 'bg-stone-50', text: 'text-stone-500', border: 'border-stone-200', icon: 'fa-clock', label: 'Pending' }
        };
        const c = config[status] || config.pending;
        return `<span class="badge ${c.bg} ${c.text} border ${c.border}"><i class="fas ${c.icon} text-[9px] mr-1"></i>${c.label}</span>`;
    }

    static async loadStats() {
        try {
            const r = await api.getVerifiedPlucking();
            if (r.success) {
                const records = r.records;
                const today = new Date().toISOString().split('T')[0];
                const todayKg = records.filter(x => x.plucking_date === today).reduce((s, x) => s + parseFloat(x.weight_kg), 0);
                const totalKg = records.reduce((s, x) => s + parseFloat(x.weight_kg), 0);
                const approved = records.filter(x => x.is_approved).length;
                const disputed = records.filter(x => x.approval_status === 'disputed').length;
                document.getElementById('verifiedStats').innerHTML = `
                    ...
                    <div class="stat-card bg-white rounded-2xl border border-emerald-200 p-4 shadow-sm bg-emerald-50/30"><p class="text-xs font-medium text-emerald-600 uppercase tracking-wider mb-2">✅ Approved</p><p class="text-2xl font-bold text-emerald-700">${approved}</p></div>
                    <div class="stat-card bg-white rounded-2xl border border-amber-200 p-4 shadow-sm bg-amber-50/30"><p class="text-xs font-medium text-amber-600 uppercase tracking-wider mb-2">⚠️ Disputed</p><p class="text-2xl font-bold text-amber-700">${disputed}</p></div>`;
            }
    ...
    static async loadRecords() {
        try {
            const r = await api.getVerifiedPlucking();
            if (r.success) { TeaPluckingVerified.allRecords = r.records; TeaPluckingVerified.renderRecords(TeaPluckingVerified.allRecords); }
        ...
        } catch (e) { document.getElementById('verifiedRecords').innerHTML = '<div class="bg-white rounded-2xl border border-stone-200 p-12 text-center"><i class="fas fa-exclamation-circle text-red-500 text-3xl mb-3"></i><p class="text-red-500">Failed to load records.</p></div>'; }
    }
    ...
    static renderRecords(records) {
        ...
        const rows = records.map(r => `
            <tr class="hover:bg-stone-50 transition-colors">
                ...
                <td class="px-4 py-3" data-label="Weight"><span class="font-semibold text-emerald-700">${parseFloat(r.weight_kg).toFixed(2)} kg</span>${r.is_approved && r.approved_kg && r.approved_kg !== r.weight_kg ? `<br><span class="text-xs text-sky-600">Approved: ${parseFloat(r.approved_kg).toFixed(2)} kg</span>` : ''}</td>
                <td class="px-4 py-3" data-label="Approval">${TeaPluckingVerified.getApprovalBadge(r)}</td>
                ...
                <td class="px-4 py-3" data-label="Actions" onclick="event.stopPropagation()">
                    <div class="flex gap-1">
                        ...
                        ${r.approval_status === 'disputed' ? `<button onclick="TeaPluckingVerified.resolveDispute('${r.id}', ${r.weight_kg})" class="w-7 h-7 flex items-center justify-center rounded-lg text-stone-400 hover:text-sky-600 hover:bg-sky-50" title="Resolve"><i class="fas fa-gavel text-xs"></i></button>` : ''}
    ...
    static filterRecords() {
        ...
        const ap = document.getElementById('vApprovalFilter')?.value || '';
        ...
        if (ap) f = f.filter(r => r.approval_status === ap);
        TeaPluckingVerified.renderRecords(f);
    }

    static clearFilters() { ['verifiedSearch','vDateFrom','vDateTo','vCompanyFilter','vApprovalFilter'].forEach(id => { const el = document.getElementById(id); if (el) el.value = ''; }); TeaPluckingVerified.renderRecords(TeaPluckingVerified.allRecords); }
    ...
    static exportCSV() {
        const h = ['Date','Worker','Company','Block','Weight (kg)','Grade','Approval','Recorded By'];
        const rows = TeaPluckingVerified.allRecords.map(r => [r.plucking_date, r.tea_workers?.full_name||'', r.companies?.name||'', r.blocks?.name||'', r.weight_kg, r.field_grade||'', r.approval_status||'pending', r.users?.username||'Unknown']);
    ...
    static async showAddForm() {
        try {
            const [wr, cr, br] = await Promise.all([api.getTeaWorkers(), api.getCompanies(), api.getBlocks()]);
            TeaPluckingVerified.allWorkers = wr.workers.filter(w => w.is_active);
            const co = cr.companies.filter(c => c.is_active).map(c => `<option value="${c.id}">${c.name} (KES ${c.buying_rate}/kg)</option>`).join('');
            const bo = br.blocks.filter(b => b.is_active).map(b => `<option value="${b.id}">${b.name}</option>`).join('');
            modal.openForm('Record Verified Plucking', `
                <div><label class="block text-sm font-semibold text-slate-700 mb-1.5">Search Worker *</label><div class="relative"><i class="fas fa-search absolute left-3.5 top-1/2 -translate-y-1/2 text-stone-400 text-sm"></i><input type="text" id="vWorkerSearch" class="w-full pl-10 pr-4 py-2.5 border-2 border-stone-200 rounded-xl text-sm focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none" placeholder="Search by name, phone or ID number..." oninput="TeaPluckingVerified.searchWorkers()"></div><input type="hidden" id="vWorker" required><div id="vWorkerSearchResults" class="mt-2 max-h-48 overflow-y-auto space-y-1"></div><div id="vWorkerStatus" class="mt-2"></div></div>
                <div id="selfReportedData" class="hidden bg-sky-50 border border-sky-200 rounded-xl p-4 mt-2"><h4 class="text-xs font-semibold text-sky-700 uppercase tracking-wider mb-3"><i class="fas fa-user-edit mr-1"></i> Self-Reported Data</h4><div id="selfReportedContent" class="space-y-2 text-sm"></div><button type="button" onclick="TeaPluckingVerified.useSelfReportedData()" class="mt-3 w-full bg-sky-600 hover:bg-sky-700 text-white py-2 rounded-lg text-sm font-medium transition-colors"><i class="fas fa-copy mr-1.5"></i> Use This Data</button></div>
                <div class="border-t border-stone-200 pt-4 mt-4"><p class="text-xs font-semibold text-stone-500 uppercase tracking-wider mb-3">Verified Data</p>
                    <div class="space-y-3">
            ...
            `, async () => {
                ...
                try {
                    ...
                    if (resp.success) {
                        modal.close();
                        const approvalMsg = resp.approval ? ` (${resp.approval.message})` : '';
                        showToast(`Verified plucking recorded!${approvalMsg}`, resp.approval?.is_approved ? 'success' : 'warning');
                        await TeaPluckingVerified.loadRecords();
                        await TeaPluckingVerified.loadStats();
                    }
    ...
    static searchWorkers() {
        ...
        if (!rd) return; if (sdd) sdd.classList.add('hidden');
    ...
    static async selectWorker(workerId, workerName) {
        ...
        const sd = document.getElementById('vWorkerStatus'); if (sd) sd.innerHTML = '';
    ...
    static onDateChange() { const wid = document.getElementById('vWorker')?.value; const date = document.getElementById('vDate')?.value; if (wid && date) { TeaPluckingVerified.loadSelfReportedData(wid, date); TeaPluckingVerified.checkExistingVerification(wid, date); } }

    static async loadSelfReportedData(workerId, date) {
        const sdd = document.getElementById('selfReportedData'); const sdc = document.getElementById('selfReportedContent');
        if (!sdd || !sdc) return;
        try {
            const resp = await api.checkWorkerPlucking(workerId, date);
            if (resp.success && resp.records.length > 0) { sdd.classList.remove('hidden'); sdc.innerHTML = resp.records.map(r => `<div class="flex justify-between items-center bg-white rounded-lg p-2.5 border border-sky-100"><div class="text-xs text-stone-600"><span class="font-medium">${r.weight_kg} kg</span> • ${r.companies?.name||'N/A'} • ${r.blocks?.name||'N/A'}${r.field_grade?' • Grade: '+r.field_grade:''}</div></div>`).join(''); TeaPluckingVerified._selfReportedData = resp.records[0]; }
    ...
    static useSelfReportedData() {
        const d = TeaPluckingVerified._selfReportedData; if (!d) { showToast('No self-reported data available.', 'warning'); return; }
        document.getElementById('vCompany').value = d.company_id || ''; document.getElementById('vBlock').value = d.block_id || ''; document.getElementById('vWeight').value = d.weight_kg || ''; document.getElementById('vGrade').value = d.field_grade || ''; document.getElementById('vNotes').value = d.notes || '';
    ...
    static async checkExistingVerification(workerId, date, workerName) {
        const sd = document.getElementById('vWorkerStatus'); if (!workerId || !date || !sd) return false;
        const ex = TeaPluckingVerified.allRecords.find(r => r.worker_id === workerId && r.plucking_date === date);
        if (ex) { const by = ex.users?.username || 'another user'; sd.innerHTML = `<div class="bg-amber-50 border border-amber-200 rounded-xl p-3 mt-2"><div class="flex items-center gap-2"><i class="fas fa-exclamation-triangle text-amber-500"></i><span class="text-sm text-amber-700 font-medium">Already verified!</span></div><p class="text-xs text-amber-600 mt-1 ml-7">${ex.weight_kg} kg - ${ex.companies?.name||'N/A'} (by: ${by}) - ${ex.approval_status||'pending'}</p><button onclick="TeaPluckingVerified.showEditForm('${ex.id}')" class="text-xs text-amber-700 underline mt-1 ml-7 hover:text-amber-800"><i class="fas fa-edit mr-1"></i>Edit</button></div>`; return true; }
        else { const w = TeaPluckingVerified.allWorkers.find(w => w.id === workerId); if (w) sd.innerHTML = `<div class="bg-emerald-50 border border-emerald-200 rounded-xl p-3 flex items-center gap-2 mt-2"><i class="fas fa-check-circle text-emerald-500"></i><span class="text-sm text-emerald-700 font-medium">${workerName||w.full_name} selected</span><span class="text-xs text-emerald-600 ml-auto">Ready to verify</span></div>`; return false; }
    }

    static async resolveDispute(recordId, currentWeight) {
        modal.openForm('Resolve Dispute', `
            <div class="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-4"><p class="text-sm text-amber-700"><i class="fas fa-info-circle mr-1"></i> This record has mismatched figures. Enter the final approved weight.</p></div>
            <div><label class="block text-sm font-semibold text-slate-700 mb-1.5">Current Verified Weight</label><input type="text" value="${currentWeight} kg" disabled class="w-full px-3.5 py-2.5 border-2 border-stone-200 rounded-xl text-sm bg-stone-50 text-stone-500"></div>
            <div><label class="block text-sm font-semibold text-slate-700 mb-1.5">Approved Weight (kg) *</label><input type="number" id="resolveApprovedKg" step="0.01" value="${currentWeight}" required class="w-full px-3.5 py-2.5 border-2 border-stone-200 rounded-xl text-sm focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none"></div>
            <div><label class="block text-sm font-semibold text-slate-700 mb-1.5">Resolution Notes</label><textarea id="resolveNotes" rows="2" class="w-full px-3.5 py-2.5 border-2 border-stone-200 rounded-xl text-sm focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 outline-none" placeholder="Why this figure was chosen..."></textarea></div>
        `, async () => {
            const kg = parseFloat(document.getElementById('resolveApprovedKg').value);
            const notes = document.getElementById('resolveNotes').value;
            if (!kg || kg <= 0) { showToast('Valid weight required.', 'warning'); return; }
            try {
                const resp = await api.approveVerifiedPlucking(recordId, kg);
                if (resp.success) { modal.close(); showToast('Dispute resolved! Record approved.', 'success'); await TeaPluckingVerified.loadRecords(); await TeaPluckingVerified.loadStats(); }
            } catch (e) { showToast(e.message, 'error'); }
        }, { submitText: 'Approve', submitIcon: 'fa-gavel', icon: 'fa-gavel', size: 'max-w-md' });
    }

    static async showEditForm(recordId) {
        const rec = TeaPluckingVerified.allRecords.find(r => r.id === recordId); if (!rec) { showToast('Record not found.', 'error'); return; }
        try {
            ...
            `, async () => {
                ...
                try { const resp = await api.updateVerifiedPlucking(recordId, ud); if (resp.success) { modal.close(); showToast('Record updated!', 'success'); await TeaPluckingVerified.loadRecords(); await TeaPluckingVerified.loadStats(); } } catch (e) { showToast(e.message, 'error'); }
